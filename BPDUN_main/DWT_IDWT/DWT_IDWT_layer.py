...
class DWT_1D(Module):
    ...
    def __init__(self, wavename):
        ...
        wavelet = pywt.Wavelet(wavename)  #创建一个小波对象
        self.band_low = wavelet.rec_lo  # rec_lo,rec_hi: 重构滤波器值
        self.band_high = wavelet.rec_hi
        assert len(self.band_low) == len(self.band_high)  #判断两者是否相等
        self.band_length = len(self.band_low)  #使两者相等
        assert self.band_length % 2 == 0  #判断是否可以整除2
        self.band_length_half = math.floor(self.band_length / 2)  #除以2 取数字的下舍整数
    ...
    def get_matrix(self):
        ...
        """
        L1 = self.input_height #这是个啥,下面forward里给了
        L = math.floor(L1 / 2)
...
class DWT_2D(Module):
    ...
    def forward(self, input):
        ...
        self.get_matrix()
        # print("in 3")
        # print(type(self.matrix_low_0))
        # print(self.matrix_low_0.shape)
