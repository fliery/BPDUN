...
class DWTFunction_1D(Function):
    @staticmethod
    def forward(ctx, input, matrix_Low, matrix_High):    # 前向传导过程  Tensor → Tensor
        ctx.save_for_backward(matrix_Low, matrix_High)    # 将输入保存起来，在backward时使用
        L = torch.matmul(input, matrix_Low.t())    #  Tensor→ Tensor If both tensors are 1-dimensional, the dot product (scalar) is returned.
        H = torch.matmul(input, matrix_High.t())   # t()将Tensor进行转置
        return L, H
    @staticmethod
    def backward(ctx, grad_L, grad_H):       # 反向传导过程  Variable → Variable
        matrix_L, matrix_H = ctx.saved_variables  # 获取前面保存的参数,也可以使用self.saved_variables
...
class DWTFunction_2D(Function):
    ...
    def forward(ctx, input, matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1):
        ...
        # print(matrix_Low_0.type())
        # print(input.type())
        L = torch.matmul(matrix_Low_0, input)    # If both arguments are 2-dimensional, the matrix-matrix product is returned.
        #print("in 1")
